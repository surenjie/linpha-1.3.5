...
function ADODB_Error_Handler($dbms, $fn, $errno, $errmsg, $p1, $p2, &$thisConnection)
{
	...
	*/
    ...
    /**
     * if adodb errorhandler is used, the function $db->ErrorMsg() does not work
     * anymore
     * -> output the error to screen
     *    -> no need anymore for :
     * if(!$create) { echo $GLOBALS['db']->ErrorMsg().'<br />'; }
     */
    echo $s.'<br />';
    
    if(function_exists('linpha_log'))
    {
    	if(!defined('TOP_DIR')) { define('TOP_DIR','..'); }
    	require_once(TOP_DIR.'/functions/other.php');
    	$s = cut_string($s, 245, 0);
        linpha_log('db','error',linpha_addslashes($s));
    }
